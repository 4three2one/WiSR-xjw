...
#     def forward_g(self, x):
#         # learning gesture network on randomized style
#         return self.classifier_g(self.randomize(self.featurizer(x)))
#
#     def forward_s(self, x):
#         # learning style network on randomized gesture
#         return self.classifier_s(self.randomize(self.featurizer(x), "gesture"))
...
#             _x = x[idx_swap]
#
#             _mean = _x.mean(-1, keepdim=True)
#             _var = _x.var(-1, keepdim=True)
#
#             _x = (_x - _mean) / (_var + eps).sqrt()
#             x = _x * (var + eps).sqrt() + mean
#
#         return x.view(*sizes)
#
#     def update(self, minibatches, unlabeled=None):
